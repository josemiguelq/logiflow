...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const delayPhase = order.status === 'PREPARING' ? 'preparing' : 'transit'
    ...
    <div className="mx-auto max-w-2xl p-6">
      ...
      {delayLevel !== 'none' && (
        ...
        >
          ...
          <span>
            Pedido atrasado
            {order.delayMinutes != null && <> há <strong>{Math.floor(order.delayMinutes)} min</strong></>}{' '}
            {delayPhase === 'preparing' ? 'em preparação' : 'em rota'}.
          </span>
        </div>
      )}
      ...
      >
        <div className="flex items-start justify-between">
          ...
          <div className="flex flex-col items-end gap-1.5">
            ...
            <DelayFlag level={delayLevel} minutes={order.delayMinutes} phase={delayPhase} />
          </div>
        </div>
